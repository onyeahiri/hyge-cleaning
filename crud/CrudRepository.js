...
class CrudRepository{
  ...
  findJoin(tables={table:[{}]}, rel={}, where=[], orderBy={table:"", column:'', order: ''}){
    return new Promise((resolve, reject)=>{
      let query = "SELECT ";
      let joinClause = '';
      ...
      if(orderBy.table && orderBy.column && orderBy.order){
        query += ` ORDER BY \`${orderBy.table}\`.\`${orderBy.column}\` ${orderBy.order}`;
      }
      query += ';';
      //console.log(query);
      this.execute(query, null)
