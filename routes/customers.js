...
router.post('/signup', (req, res) => {
  ...
  customerRepository.findOne([{ email_address: customer.email_address }])
    .then(emailExists => {
      if (emailExists) {
      ...
      customerRepository.findOne([{ phone_no: customer.phone_no }])
        .then(phoneNoExists => {
          if (phoneNoExists) {
          ...
          bcrypt.genSalt(12, (err, salt)=>{
            ...
            bcrypt.hash(customer.password, salt, (err, hash)=>{
              ...
              customerRepository.insert(customer)
                .then(inserted=>{
                  req.flash('success', inserted.name + ' thank you for creating  account with us');
        ...
        .then(service => {
          ...
          stripe.charges.create({
            ...
            .then(info => {
              ...
              transactionRepository.insert(newTransact)
                .then(transact => {
                  //-- send mail

                  req.flash("success", `${req.user.name} your service request has been saved.`);
                  res.redirect("/customer/booking/");
                })
                .catch(err => {
                  console.log(err);
